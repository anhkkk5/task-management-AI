...
exports.aiService = {
    ...
    taskBreakdown: async (userId, input) => {
        ...
        const isBeginner = levelCode === "intern" ||
            levelCode === "fresher" ||
            levelCode === "student";
        const isProfessional = levelCode === "junior" ||
            levelCode === "middle" ||
            levelCode === "senior" ||
            levelCode === "lead" ||
            levelCode === "manager" ||
            levelCode === "pm";
        const totalHint = input.totalMinutes && input.totalMinutes > 0
        ...
        const profileDomainText = `${input.profile?.positionCode ?? ""} ${input.profile?.positionLabel ?? ""}`.toLowerCase();
        const frontendHints = [
            "frontend",
            "front-end",
            "ui",
        ...
        const titleIndicatesUiOnly = [
            ...
            "form",
            "login/register ui",
        ].some((k) => String(input.title || "")
            .toLowerCase()
            .includes(k));
        const roleIsFrontend = ["frontend", "front-end", "ui", "ux", "react"].some((k) => profileDomainText.includes(k));
        const roleIsBackend = ["backend", "back-end", "api", "server"].some((k) => profileDomainText.includes(k));
        const roleIsFullstack = ["fullstack", "full-stack"].some((k) => profileDomainText.includes(k));
        const domainMode = titleIndicatesUiOnly
            ? "frontend_strict"
...
- Tổng thời lượng cho 1 task feature-level thông thường chỉ nên rơi vào 2-5 giờ, trừ khi task thực sự lớn.`
        ...
        const titleTokens = new Set(tokenize(input.title).filter((t) => t.length >= 3 && !stopWords.has(t)));
        const descTokensList = tokenize(descriptionRaw).filter((t) => t.length >= 3 && !stopWords.has(t));
        ...
        for (const t of descTokens)
            if (titleTokens.has(t))
                sharedTokenCount += 1;
        const descHasFrontendHint = frontendHints.some((k) => descriptionRaw.toLowerCase().includes(k));
        const descHasBackendHint = backendHints.some((k) => descriptionRaw.toLowerCase().includes(k));
        const domainMatchesTitle = (hasFrontendHint && descHasFrontendHint) ||
            (hasBackendHint && descHasBackendHint);
        const specHintRegex = /(acceptance|tiêu chí|input|output|payload|endpoint|schema|field|trường|response|status|validate|kpi|deliverable|yêu cầu|ràng buộc|api|jwt|token|oauth|refresh|login|register)/i;
        const descHasSpecHints = specHintRegex.test(descriptionRaw);
        const descriptionRelevant = descWordCount >= 4 &&
            (sharedTokenCount >= 1 || domainMatchesTitle || descHasSpecHints);
        if (descriptionRaw && !descriptionRelevant) {
            console.log(`[aiBreakdown] Description bị bỏ qua vì không liên quan title "${input.title}" (words=${descWordCount}, shared=${sharedTokenCount})`);
        }
        const descriptionText = descriptionRelevant
            ? `Mô tả (SPEC ĐÁNG TIN của task, BẮT BUỘC bám theo): ${descriptionRaw}`
            : "";
        const anchorKeywords = Array.from(new Set(descriptionRelevant
            ? [...titleTokens, ...descTokensList]
            : Array.from(titleTokens))).slice(0, descriptionRelevant ? 20 : 12);
        const anchorSet = new Set(anchorKeywords);
        const groundingGuidance = anchorKeywords.length > 0
...
- KHÔNG bịa chi tiết ngoài mô tả; nếu mô tả không nêu, chỉ bổ sung bước tiêu chuẩn tối thiểu (thiết kế schema, test, review).`
            : descriptionRaw
                ...
                : "";
        const buildPrompt = (extraGuidance = "") => `Hãy breakdown công việc sau thành một QUY TRÌNH LÀM VIỆC (workflow) có thứ tự, mỗi bước là MỘT hành động/deliverable cụ thể.
...
${extraGuidance}`;
        const generateSteps = async (extraGuidance = "") => {
            ...
            try {
                ...
                try {
                    ...
                    console.error("[aiBreakdown] AI_JSON_INVALID. Raw response (first 500 chars):", raw.slice(0, 500));
        ...
        try {
            ...
            if (firstErr?.message === "AI_JSON_INVALID") {
                console.warn("[aiBreakdown] First attempt failed (AI_JSON_INVALID). Retrying with stricter prompt...");
        ...
        const hasOutOfScopeAdvancedOps = !explicitlyRequestsAdvancedOps &&
            steps.some((step) => {
                const text = `${step.title} ${step.description ?? ""}`.toLowerCase();
        ...
        if (descriptionRelevant && descriptionRaw) {
            ...
            for (const e of endpoints)
                criticalLiterals.push(e.trim());
            // 2) Số + đơn vị: "15 phút", "7 ngày", "8 ký tự", "10 giây"
            const numUnitRe = /\d+\s*(?:phút|giây|ngày|tuần|tháng|giờ|ký tự|characters?|bytes?|mb|kb)/gi;
            const numUnits = descriptionRaw.match(numUnitRe) ?? [];
            for (const n of numUnits)
                criticalLiterals.push(n.trim());
            ...
            for (const id of ids)
                criticalLiterals.push(id);
            // 4) JSON shape: { ... , ... } có ít nhất 2 phần
            const shapeRe = /\{[^{}\n]{3,}\}/g;
            const shapes = descriptionRaw.match(shapeRe) ?? [];
            for (const sh of shapes)
                criticalLiterals.push(sh.trim());
        }
        const uniqueLiterals = Array.from(new Set(criticalLiterals)).filter((s) => s.length > 0);
        const missingLiterals = uniqueLiterals.filter((lit) => {
            ...
            if (needle.startsWith("{")) {
                ...
                const ids = (needle.match(/[a-z_][a-z0-9_]+/g) ?? []).filter((x) => x.length >= 3);
                if (ids.length === 0)
        ...
        const needsRetry = (hasTaskAnchors && ungroundedCount > Math.floor(steps.length / 2)) ||
        ...
        if (needsRetry) {
            ...
            if ((hasTaskAnchors && ungroundedCount > Math.floor(steps.length / 2)) ||
                hasCrossDomainMismatch ||
                hasOutOfScopeAdvancedOps) {
                parts.push(`- Viết lại breakdown bám sát mục tiêu task, loại bỏ mọi bước ngoài phạm vi.`);
                parts.push(`- Tập trung theo các từ khóa chính: ${anchorKeywords.join(", ") || input.title}.`);
                parts.push(`- TUYỆT ĐỐI không thêm deploy/production/2FA/audit/lockout nếu task không yêu cầu.`);
            }
            if (missingLiterals.length > 0) {
                parts.push(`- BẮT BUỘC phản ánh ĐẦY ĐỦ các chi tiết sau từ mô tả (đưa nguyên văn vào title/description của bước phù hợp, KHÔNG diễn đạt lại mơ hồ):`);
                for (const lit of missingLiterals) {
                    parts.push(`  • ${lit}`);
                }
                parts.push(`- Nếu có response shape JSON, ghi nguyên văn shape đó trong description của bước tương ứng.`);
                parts.push(`- Nếu có endpoint cụ thể, ĐƯA NGUYÊN VĂN endpoint vào title của bước implement API đó.`);
            }
            const retryGuidance = "\n" + parts.join("\n");
            console.log(`[aiBreakdown] Retry due to ${missingLiterals.length > 0 ? `missing literals: ${missingLiterals.join(" | ")}` : "grounding issue"}`);
