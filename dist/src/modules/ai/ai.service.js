        ...
        const constrainedSchedule = normalizedSchedule
            ...
            return {
                ...
                tasks: filteredTasks.map((t) => ({
                    ...
                    sessionId: t?.sessionId !== undefined && t?.sessionId !== null
                        ? String(t.sessionId)
                        : String(`${String(t?.taskId ?? "")}:${String(day.date)}:${String(t?.suggestedTime ?? "")}`),
                })),
            };
        });
  ...
  "advice": "Lời khuyên để tránh bỏ lỡ lần sau (1-2 câu)"
        ...
        return {
            suggestion: {
                ...
                reason: String(parsed.suggestion.reason || "Đề xuất dựa trên thói quen của bạn"),
                confidence: ["high", "medium", "low"].includes(parsed.suggestion.confidence)
                    ? parsed.suggestion.confidence
                    : "medium",
            },
            alternativeSlots: Array.isArray(parsed.alternativeSlots)
                ? parsed.alternativeSlots.map((slot) => ({
