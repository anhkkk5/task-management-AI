...
exports.subtaskChatService = void 0;
...
exports.subtaskChatService = {
    chat: async (input) => {
        ...
        const activeSchedules = await ai_schedule_model_1.AISchedule.find({
            userId: new mongoose_1.Types.ObjectId(userId),
            isActive: true,
        ...
        const completedCount = targetTask.aiBreakdown.filter((s) => s.status === "completed").length;
        const totalSubtasks = targetTask.aiBreakdown.length;
        // 4. Xây dựng lịch học chi tiết để đưa vào context
        const scheduleContext = slots.length > 0
...
function buildSystemPrompt(ctx) {
    const { taskTitle, taskDescription, taskEstimatedDuration, taskDeadline, subtask, subtaskIndex, totalSubtasks, completedCount, scheduleContext, slots, } = ctx;
    ...
    const durationStr = taskEstimatedDuration
        ...
        : "";
    const slotDuration = subtask.scheduledDate && slots.length > 0
