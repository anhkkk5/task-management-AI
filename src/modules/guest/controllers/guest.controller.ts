...
const parsePermission = (
  value: unknown,
): "edit_event" | "view_guest_list" | "invite_others" | undefined => {
  ...
  try {
    ...
    if (
      ...
      res.status(401).json({
        success: false,
        error:
          "Cần xác thực Google. Vui lòng kết nối tài khoản Google của bạn.",
      });
      return;
    }
    ...
    if (message.includes("temporarily unavailable")) {
      res.status(503).json({
        success: false,
        error:
          "Dịch vụ Google Contacts tạm thời không khả dụng. Vui lòng thử lại sau.",
      });
      return;
    }
  ...
  try {
    ...
    if (!permission) {
      res.status(400).json({
        success: false,
        error:
          "Quyền không hợp lệ. Phải là: edit_event, view_guest_list, invite_others",
      });
      return;
    }

    // Update guest permission
    const guest = await guestService.updateGuestPermission(guestId, permission);
