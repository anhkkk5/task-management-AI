...
/**
 * Contact interface representing a Google Contact
 * @interface Contact
 */
...
export class ContactSearchService {
  private readonly GOOGLE_PEOPLE_API_BASE_URL =
    "https://people.googleapis.com/v1";
  private readonly GOOGLE_OAUTH_TOKEN_URL =
  ...
  async searchContacts(
    ...
    try {
      ...
      let contacts = await this.searchContactsWithToken(
      ...
      if (contacts === null) {
        ...
        if (!newToken) {
          throw new Error(
            "Failed to refresh Google authentication. Please re-authenticate.",
          );
        }

        contacts = await this.searchContactsWithToken(
          newToken,
          searchTerm,
          limit,
          offset,
   ...
   */
  private async searchContactsWithToken(
    accessToken: string,
    searchTerm: string,
    limit: number,
    offset: number,
    ...
    try {
      ...
      const response = await axios.get(
        `${this.GOOGLE_PEOPLE_API_BASE_URL}/people/me/connections`,
        {
          params: {
      ...
      console.log(
        `[ContactSearchService] Search term: ${searchTerm}, Found: ${filteredContacts.length} contacts`,
      );
      ...
      if (filteredContacts.length === 0 && this.isValidEmail(searchTerm)) {
        console.log(
          `[ContactSearchService] Creating placeholder contact for email: ${searchTerm}`,
        );
      ...
      console.error(
        `[ContactSearchService] Error searching contacts:`,
        axiosError.message,
      );
      ...
      if (this.isValidEmail(searchTerm)) {
        console.log(
          `[ContactSearchService] Google API failed, creating placeholder contact for email: ${searchTerm}`,
        );
        const placeholderContact: Contact = {
          id: `email_${searchTerm}`,
          email: searchTerm.toLowerCase(),
  ...
  private parseContactsFromResponse(responseData: any): Contact[] {
    ...
    return responseData.connections
      .map((person: any) => {
        ...
        const phoneNumbers = person.phoneNumbers?.map(
   ...
   */
  private async refreshGoogleToken(user: any): Promise<string | null> {
    try {
      ...
      console.warn(
        `[ContactSearchService] Token refresh not implemented for user ${user._id}`,
      );
      return null;
    } catch (error) {
   ...
   */
  private async getUserWithToken(
    userId: string | Types.ObjectId,
  ): Promise<any> {
    try {
      // Import User model dynamically to avoid circular dependencies
      const { User: UserModel } = await import("../../auth/auth.model");

      const user =
        await UserModel.findById(userId).select("+googleAccessToken");
  ...
  private handleError(error: any): Error {
    ...
    if (axiosError.response) {
      ...
      switch (status) {
        case 400:
          return new Error(
            `Invalid search request: ${data?.error?.message || "Bad request"}`,
          );
        case 401:
          return new Error(
        ...
        case 403:
          return new Error(
            "Permission denied to access Google Contacts. Please check your Google account permissions.",
          );
        case 429:
          return new Error(
            "Too many requests to Google API. Please try again later.",
          );
        case 500:
        case 503:
          return new Error(
            "Google Contacts service is temporarily unavailable. Please try again later.",
          );
        default:
          return new Error(
