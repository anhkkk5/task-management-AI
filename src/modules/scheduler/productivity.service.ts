...
export class ProductivityScorer {
  ...
  calculateHourlyScores(
    ...
    const hourlyStats = new Map<number, { completed: number; total: number; totalDuration: number }>();
  ...
  analyzeTrend(
    ...
    const recentAvg = recent.map(day => 
      Array.from(day.values()).reduce((sum, s) => sum + s.score, 0) / day.size
    );
    const olderAvg = older.map(day => 
      Array.from(day.values()).reduce((sum, s) => sum + s.score, 0) / day.size
    );
