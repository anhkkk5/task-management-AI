...
import { UserMemoryDoc, UserMemoryScope } from "./user-memory.model";

/**
 * Domain-agnostic rule-based extractor.
 * We detect stable user preferences/facts from a single user utterance.
 *
 * Principles:
 *  - Only extract when signal is very high (regex-anchored) to avoid noise.
 *  - Key is a canonical lowercase snake_case slug.
 *  - Value is the raw matched token (normalized trim).
 *  - Domain is coarse ("it", "language_learning", etc.) when detectable,
 *    otherwise undefined (still stored as general preference).
 *
 * To extend for other domains, append a detector below.
 */
...
const detectDomain = (text: string): string | undefined => {
  for (const d of DOMAIN_PATTERNS) {
    if (d.re.test(text)) return d.domain;
  }
  return undefined;
};

// ─── Preference detectors ─────────────────────────────────────────────────────
// Each returns array of candidates for the given utterance.
type Detector = (text: string, domain?: string) => ExtractedMemory[];
...
const detectTechStack: Detector = (text) => {
...
const detectProgrammingLanguage: Detector = (text) => {
...
const detectSpokenLanguage: Detector = (text) => {
...
const detectTool: Detector = (text) => {
...
// "tôi là/i am X (role)"
const detectRole: Detector = (text) => {
  const m =
    /(?:tôi là|mình là|em là|i am|i'm)\s+(?:một\s+|a\s+|an\s+)?([\p{L}\s-]{3,40}?)(?:[.,!?]|$)/iu.exec(
      text,
    );
  if (!m) return [];
...
export const extractMemoriesFromUtterance = (
  text: string,
): ExtractedMemory[] => {
...
export const userMemoryService = {
   ...
   */
  ingestUtterance: async (
    userId: Types.ObjectId,
    text: string,
  ): Promise<void> => {
    try {
      const memories = extractMemoriesFromUtterance(text);
      for (const m of memories) {
        await aiRepository.upsertUserMemory({
          userId,
          scope: m.scope,
          key: m.key,
          value: m.value,
  ...
  loadRelevantMemories: async (
    ...
    // First domain-scoped
    const scoped = domain
      ? await aiRepository.listUserMemories({
          userId,
    ...
    const rest = await aiRepository.listUserMemories({
      userId,
      scopes: ["preference", "fact"],
