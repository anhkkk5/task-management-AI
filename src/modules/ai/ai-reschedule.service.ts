...
function determineConfidence(
  score: number,
  hasBusyConflicts: boolean,
): "high" | "medium" | "low" {
  if (score >= 0.75 && !hasBusyConflicts) return "high";
...
const ADVICE_MAP: Record<string, string> = {
  missed:
    "Hãy đặt nhắc nhở trước 10 phút và bắt đầu đúng giờ để tạo thói quen tốt.",
  overlapping:
    "Khi lên lịch, hãy để buffer 15 phút giữa các task để tránh trùng lịch.",
  too_short:
    "Hãy ước lượng thời gian thực tế hơn khi tạo task — thêm 20% buffer cho bất ngờ.",
  manual:
    "Nếu thường xuyên hoãn task này, hãy chia nhỏ thành các bước dễ bắt đầu hơn.",
};
  ...
  smartReschedule: async (
    ...
    if (reason === "too_short" && missedTask.originalScheduledTime) {
      const origDuration = Math.round(
        (new Date(missedTask.originalScheduledTime.end).getTime() -
    ...
    const searchEnd = new Date(
      Math.min(deadline.getTime(), now.getTime() + 7 * 24 * 60 * 60 * 1000),
    );
    ...
    const scheduledTasks = await taskRepository.getScheduledTasks({
      ...
      startDate: now,
      endDate: searchEnd,
      excludeTaskIds: [missedTask.id].filter((id) =>
    ...
    while (currentDate <= searchEnd && scoredSlots.length < 10) {
      ...
      const dayBusy: TimeInterval[] = scheduledTasks
        .filter((t: any) => {
          if (!t.scheduledTime?.start) return false;
      ...
      const outsideBusy = buildOutsideAvailabilityBusySlots(
        new Date(currentDate),
        availableSlots,
      );
      ...
      if (optimalSlot) {
        const slotEnd = new Date(
          optimalSlot.start.getTime() + duration * 60 * 1000,
        );
        // Verify no conflict
        const conflict = intervalScheduler.checkConflict(
          { start: optimalSlot.start, end: slotEnd, taskId: missedTask.id },
          allBusy,
        );
      ...
      for (const free of freeSlots) {
        ...
        const isDuplicate = scoredSlots.some(
          (s) =>
            s.start.getTime() === free.start.getTime() && s.date === dateStr,
        );
        if (!isDuplicate && scoredSlots.length < 10) {
          const prodScore =
            productivityScores.get(free.start.getHours())?.score ?? 0.5;
          scoredSlots.push({
            start: free.start,
            end: slotEnd,
            date: dateStr,
    ...
    if (scoredSlots.length === 0) {
      return {
        suggestion: {
          newStartTime: "09:00",
          newEndTime: formatHHMM(
            new Date(
              new Date(`${todayStr}T09:00:00`).getTime() + duration * 60 * 1000,
            ),
          ),
          newDate: todayStr,
